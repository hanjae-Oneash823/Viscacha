...
_GTF    = REPO_ROOT / "outputs/annotation/extended_annotation.filtered.gtf"
_GENOME = REPO_ROOT / "outputs/reference/GRCh38.primary_assembly.genome.fa"
...
def load_cache() -> dict[str, str]:
    """Return the full cached {transcript_name: cds_sequence} map, unfiltered.

    Unlike run(), never triggers a fresh TransDecoder execution -- used by
    the SQANTI3-vs-TransDecoder conflict check in j1_sequences.py, which only
    wants to compare against whatever's already been predicted, not force
    prediction on transcripts SQANTI3 already resolved.
    """
