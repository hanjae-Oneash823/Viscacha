...
Row-level filter (run() end, _drop_protein_identical_alts): trial_failure
non-canonical alt rows classified identical/no_sequence are protein-level
decoys -- dropped outright, not just flagged. Any trial_failure hit left
with zero non-canonical alt rows afterward (every ranked alternate turned
out protein-identical to canonical) is dropped entirely -- it has no
replacement candidate to tell a docking story about. Downstream (J4), every
surviving trial_failure alt row already has a real protein change by
construction.
"""
...
def _drop_protein_identical_alts(df: pd.DataFrame) -> pd.DataFrame:
    """A trial_failure non-canonical alt row whose protein_change_type is
    null (identical/no_sequence -- NULL_PROTEIN_CHANGE_TYPES) is a decoy: the
    transcript is DTU-distinct and usage-significant, but translates to the
    same protein as canonical (e.g. a UTR-only splice difference), so there
    is nothing to structurally compare or dock. Drop those rows, then drop
    any trial_failure hit left with zero non-canonical alt rows at all -- a
    canonical-only hit has no replacement candidate left and can never tell
    a docking story, so it's removed entirely (mirrors J1c's has_viable_alt
    check, just re-run now that protein_change_type exists).
    """
    tf_mask = df["candidate_group"] == "trial_failure_candidate"
    noncanon_mask = tf_mask & (df["is_canonical"] != True)  # noqa: E712
    decoy = noncanon_mask & df["protein_change_type"].isin(NULL_PROTEIN_CHANGE_TYPES)

    _log(f"dropping {decoy.sum():,} trial_failure alt rows with a protein-identical "
         f"change ({sorted(NULL_PROTEIN_CHANGE_TYPES)}) -- transcript differs, protein doesn't")
    df = df[~decoy].copy()

    tf = df[df["candidate_group"] == "trial_failure_candidate"]
    tf_keys = set(map(tuple, tf[["gene_name", "cell_type"]].drop_duplicates().values))
    alt_keys = set(map(
        tuple,
        tf.loc[tf["is_canonical"] != True, ["gene_name", "cell_type"]]  # noqa: E712
        .drop_duplicates().values,
    ))
    empty_hits = tf_keys - alt_keys
    if empty_hits:
        drop_hit = (df["candidate_group"] == "trial_failure_candidate") & df.apply(
            lambda r: (r["gene_name"], r["cell_type"]) in empty_hits, axis=1
        )
        _log(f"dropping {len(empty_hits):,} trial_failure hits "
             f"({drop_hit.sum():,} rows) with no protein-changing alt left")
        df = df[~drop_hit].copy()
...
def run(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out = _drop_protein_identical_alts(out)
