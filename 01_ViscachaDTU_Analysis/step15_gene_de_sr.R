...
#
# Same primary model as the DTU analysis and step15_gene_de.R
...
run_gene_de_one <- function(cell_type) {
  f_counts <- file.path(IN_DIR_SR, paste0("counts_", cell_type, ".csv"))
  f_meta   <- file.path(IN_DIR_SR, paste0("metadata_", cell_type, ".csv"))
  if (!file.exists(f_counts) || !file.exists(f_meta)) {
    message("  [", cell_type, "] SR pseudobulk not found — skipping")
    return(NULL)
  }
...
cat(strrep("=", 60), "\n")
cat("Step 15 (SR) — Gene-level DE (DESeq2) on short-read pseudobulk\n")
cat(strrep("=", 60), "\n")
...
out_path <- file.path(OUT_DIR, "gene_level_de_results_SR.csv")
