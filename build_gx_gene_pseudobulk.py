#!/usr/bin/env python3
"""
Build gene-level pseudobulk (donors x genes) from the LONG-READ GENE-LEVEL
h5ad (adata_gene_loose_filtering_for_bulk_analysis.h5ad, "GX"), mirroring
build_sr_gene_pseudobulk.py's donor-aggregation logic exactly (same
MIN_CELLS_PB, same primary/sensitivity donor split), so its output is a
drop-in gene-level counterpart to the short-read pseudobulk used by
step15_gene_de_sr.R.
...
def main() -> None:
    OUT_PB_GX.mkdir(parents=True, exist_ok=True)

    print("Loading unified_metadata.csv ...")
    unified_meta = pd.read_csv(OUT_META / "unified_metadata.csv", index_col="donor_id")

    print(f"Loading adata_gene_loose from {GX_PATH} ...")
    adata = ad.read_h5ad(GX_PATH)
    print(f"  {adata.shape[0]} cells x {adata.shape[1]} genes")

    # GX uses 'sample' where TX/SR use 'donor'; rename for parity.
    adata.obs["donor"] = adata.obs["sample"]

    # GX already has pct_counts_mt/condition/cell_type. Only age/sex/braak_stage
    # need joining, same as build_sr_gene_pseudobulk.py.
    ...
    for ct in CELL_TYPES:
        ...
        if len(primary_donors) > 0:
            count_df.loc[primary_donors].to_csv(OUT_PB_GX / f"counts_{stem}.csv")
            meta_df.loc[primary_donors].to_csv(OUT_PB_GX / f"metadata_{stem}.csv")
        if len(sensitivity_donors) > 0:
            count_df.loc[sensitivity_donors].to_csv(OUT_PB_GX / f"counts_{stem}_sensitivity.csv")
            meta_df.loc[sensitivity_donors].to_csv(OUT_PB_GX / f"metadata_{stem}_sensitivity.csv")

        print(f"  [{ct}] {len(primary_donors)} primary + {len(sensitivity_donors)} sensitivity donors, "
              f"{count_df.shape[1]} genes -> saved as '{stem}'")

    print(f"\nDone. GX gene pseudobulk written to {OUT_PB_GX}")
