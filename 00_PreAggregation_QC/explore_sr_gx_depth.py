...
and adata_gene_loose_filtering_for_bulk_analysis (long-read, gene-level). Both
raw inputs already carry a precomputed `total_counts` in .obs, so this never
touches the (37 GB) adata_sr.X matrix -- only .obs is read, in backed mode.

The GX file has no `donor` column, only `sample`; for matched barcodes
`sample` plays the same role as `donor` in the TX file, so it is renamed to
`donor` here for parity with explore_sr_tx_depth.py.
...
def load_obs() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    print("Loading adata_gene_loose (backed, obs only)...")
    gx = ad.read_h5ad(GX_PATH, backed="r")
    gx_obs = gx.obs[["sample", "condition", "cell_type", "library_kit", "total_counts",
                      "n_genes_by_counts"]].copy()
    gx_obs = gx_obs.rename(columns={"sample": "donor"})
    gx.file.close()

    return sr_obs, gx_obs


def build_merged(sr_obs: pd.DataFrame, gx_obs: pd.DataFrame) -> pd.DataFrame:
    merged = sr_obs.join(gx_obs, lsuffix="_sr", rsuffix="_gx", how="inner")
    merged["ratio_sr_to_gx"] = (
        merged["total_counts_sr"] / merged["total_counts_gx"].replace(0, np.nan)
    )
    return merged


# ---------------------------------------------------------------------------
# 1. SR vs GX depth — hexbin scatter, log-log, with correlation annotated
# ---------------------------------------------------------------------------
def plot_depth_scatter(merged: pd.DataFrame) -> None:
    r_raw, _   = pearsonr(merged["total_counts_sr"], merged["total_counts_gx"])
    r_log, _   = pearsonr(np.log1p(merged["total_counts_sr"]),
                           np.log1p(merged["total_counts_gx"]))
    rho, _     = spearmanr(merged["total_counts_sr"], merged["total_counts_gx"])

    fig, ax = plt.subplots(figsize=(7, 6))
    hb = ax.hexbin(merged["total_counts_sr"], merged["total_counts_gx"],
    ...
    ax.set_ylabel("GX (long-read gene-level) total_counts (log scale)")
    ax.set_title("Per-cell depth: short-read vs long-read gene-level", weight="semibold", pad=12)
    ax.text(0.03, 0.97,
    ...
    sns.despine(ax=ax)
    fig.tight_layout()
    _savefig(fig, "01_sr_vs_gx_depth_scatter.png")


# ---------------------------------------------------------------------------
# 2. Marginal histograms of SR depth and GX depth
# ---------------------------------------------------------------------------
def plot_marginal_histograms(merged: pd.DataFrame) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(11, 4.5))
    for ax, col, label, color in [
        (axes[0], "total_counts_sr", "SR total_counts", "#3A86FF"),
        (axes[1], "total_counts_gx", "GX total_counts", "#FF006E"),
    ]:
        vals = merged[col]
...
# ---------------------------------------------------------------------------
# 3. SR/GX ratio distribution
# ---------------------------------------------------------------------------
def plot_ratio_distribution(merged: pd.DataFrame) -> None:
    ratio = merged["ratio_sr_to_gx"].dropna()
    ...
    ax.set_xlabel("SR / GX depth ratio per cell (log scale)")
    ax.set_ylabel("# cells")
    ax.set_title("How many fold deeper is SR than long-read GX, per cell?", weight="semibold", pad=12)
    ax.legend(frameon=False)
    sns.despine(ax=ax)
    fig.tight_layout()
    _savefig(fig, "03_sr_gx_ratio_distribution.png")


# ---------------------------------------------------------------------------
# 4. Capture efficiency (GX/SR) vs SR depth, binned
# ---------------------------------------------------------------------------
def plot_efficiency_vs_depth(merged: pd.DataFrame) -> None:
    df = merged.copy()
    df["efficiency"] = df["total_counts_gx"] / df["total_counts_sr"]
    ...
    ax.set_ylabel("GX/SR capture efficiency (log scale)")
    ax.set_title("Does long-read (gene-level) capture efficiency change with depth?", weight="semibold", pad=12)
    ax.legend(frameon=False)
    sns.despine(ax=ax)
    fig.tight_layout()
...
def plot_by_cell_type(merged: pd.DataFrame) -> None:
    cell_types = sorted(merged["cell_type_gx"].dropna().unique())
    n = len(cell_types)
    n_col = min(4, n)
    ...
    for i, ct in enumerate(cell_types):
        ...
        sub = merged[merged["cell_type_gx"] == ct]
        r_log, _ = pearsonr(np.log1p(sub["total_counts_sr"]), np.log1p(sub["total_counts_gx"]))
        gridsize = max(10, min(40, int(np.sqrt(len(sub)))))
        ax.hexbin(sub["total_counts_sr"], sub["total_counts_gx"],
    ...
    fig.suptitle("SR vs GX depth, by cell type", weight="semibold", y=1.02)
    fig.tight_layout()
...
def plot_by_donor(merged: pd.DataFrame) -> None:
    donors = sorted(merged["donor_gx"].dropna().unique())
    n = len(donors)
    n_col = 5
    n_row = int(np.ceil(n / n_col))
    ...
    for i, donor in enumerate(donors):
        ax = axes[i // n_col][i % n_col]
        sub = merged[merged["donor_gx"] == donor]
        if len(sub) >= 3:
            r_log, _ = pearsonr(np.log1p(sub["total_counts_sr"]), np.log1p(sub["total_counts_gx"]))
    ...
    fig.suptitle("SR vs GX depth, by donor", weight="semibold", y=1.01)
    fig.tight_layout()
    _savefig(fig, "06_depth_scatter_by_donor.png")


# ---------------------------------------------------------------------------
# 7. SR depth: cells with vs without a GX match
# ---------------------------------------------------------------------------
def plot_sr_depth_by_match_status(sr_obs: pd.DataFrame, gx_obs: pd.DataFrame) -> None:
    has_match = sr_obs.index.isin(gx_obs.index)
    df = sr_obs.copy()
    df["has_gx_match"] = np.where(has_match, "Matched in GX", "No GX match")

    fig, ax = plt.subplots(figsize=(7, 5))
    bins = np.logspace(np.log10(max(df["total_counts"].min(), 1)),
                        np.log10(df["total_counts"].max()), 50)
    for label, color in [("Matched in GX", "#06D6A0"), ("No GX match", "#FB5607")]:
        sub = df[df["has_gx_match"] == label]["total_counts"]
        ax.hist(sub, bins=bins, alpha=0.55, color=color, label=f"{label} (n={len(sub):,})",
                density=True, edgecolor="white", linewidth=0.2)
    ax.set_xscale("log")
    ax.set_xlabel("SR total_counts (log scale)")
    ax.set_ylabel("density")
    ax.set_title("Are the SR cells missing from GX simply the lowest-depth ones?",
                 weight="semibold", pad=12, fontsize=11)
    ax.legend(frameon=False)
    sns.despine(ax=ax)
    fig.tight_layout()
    _savefig(fig, "07_sr_depth_by_gx_match_status.png")


def main():
    sr_obs, gx_obs = load_obs()
    merged = build_merged(sr_obs, gx_obs)

    print(f"\nn SR cells: {len(sr_obs):,}")
    print(f"n GX cells: {len(gx_obs):,}")
    print(f"n matched (inner join): {len(merged):,}")
    print(f"frac GX barcodes found in SR: {len(merged) / len(gx_obs):.3f}")
    print(f"frac SR barcodes found in GX: {len(merged) / len(sr_obs):.3f}\n")
    ...
    plot_sr_depth_by_match_status(sr_obs, gx_obs)
